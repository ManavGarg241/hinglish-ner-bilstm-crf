...
optparser.add_option(
    '--model_path', default='models/hinglish_ner',
    help='model path'
)
optparser.add_option(
...
def eval(model, datas):
    # Evaluate using seqeval for proper span-level NER evaluation
    all_true_tags = []
    all_pred_tags = []
    ...
    # Use seqeval for proper span-level NER metrics
    print("\n" + "="*80)
    print("Span-Level Classification Report:")
    print("="*80)
    print(classification_report(all_true_tags, all_pred_tags))
    
    span_f1 = f1_score(all_true_tags, all_pred_tags)
    print(f"Overall Span F1 Score: {span_f1:.4f}")
    print("="*80 + "\n")
